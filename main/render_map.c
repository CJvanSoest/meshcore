...
static bool latlon_to_fb(double lat, double lon,
                         double ctr_lat, double ctr_lon, int zoom,
                         int map_x0, int map_y0, int map_w, int map_h,
                         int *out_x, int *out_y) {
    int ctx, cty, cpx, cpy;
...
static void render_tile_raster(double lat, double lon, int zoom,
                               int map_x0, int map_y0,
                               int map_w,  int map_h) {
    int center_tx, center_ty, px_in, py_in;
    map_latlon_to_tile(lat, lon, zoom,
                       &center_tx, &center_ty, &px_in, &py_in);
    ...
    for (int dy = dy_min; dy <= dy_max; dy++) {
        for (int dx = dx_min; dx <= dx_max; dx++) {
            ...
            bool visible =
                !(tx + MAP_TILE_PX < map_x0 || tx >= map_x0 + map_w ||
                  ty + MAP_TILE_PX < map_y0 || ty >= map_y0 + map_h);
            ...
            if (t) {
                ...
                pax_simple_rect(&fb, COL_PANEL, tx, ty,
...
static void render_scale_bar(double lat, int zoom,
                             int map_x0, int map_y0, int map_w, int map_h) {
    (void)map_x0; (void)map_w;
    double lat_rad   = lat * M_PI / 180.0;
    double earth_eq  = 40075016.686;  // metres
    double m_per_px  = (earth_eq * cos(lat_rad)) /
                       ((double)MAP_TILE_PX * (double)(1 << zoom));
    ...
    char buf[16];
    if (chosen >= 1000) snprintf(buf, sizeof(buf), "%d km", chosen / 1000);
    else                snprintf(buf, sizeof(buf), "%d m",  chosen);
...
static void render_status_strip(int w, int map_y0) {
    ...
    bool     stale    = gps_live_last_fix_ms != 0 &&
                       (now - gps_live_last_fix_ms) > 60000u;
    pax_col_t sat_col = sats >= 4 ? COL_GREEN :
                        sats >  0 ? COL_AMBER : COL_RED;
    if (stale) sat_col = COL_RED;
    char buf[16];
    if (gps_live_bus_ok) snprintf(buf, sizeof(buf), "SAT:%u", (unsigned)sats);
    else                 snprintf(buf, sizeof(buf), "SAT:?");
    pax_vec2f sz = pax_text_size(FONT, TXT_SMALL, buf);
    x -= (int)sz.x;
    pax_simple_rect(&fb, 0x80000000, x - 4, y - 2, (int)sz.x + 8, TXT_SMALL + 4);
...
static pax_col_t role_pin_color(meshcore_device_role_t r) {
    switch (r) {
        case MESHCORE_DEVICE_ROLE_REPEATER:    return ROLE_COL_RPTR;
        case MESHCORE_DEVICE_ROLE_ROOM_SERVER: return ROLE_COL_ROOM;
        case MESHCORE_DEVICE_ROLE_SENSOR:      return ROLE_COL_SENSOR;
        case MESHCORE_DEVICE_ROLE_CHAT_NODE:
        default:                               return ROLE_COL_CHAT;
    }
}
...
static void render_legend(int w, int map_y0) {
    (void)w;
    static const struct { meshcore_device_role_t r; const char *l; } items[] = {
    ...
    for (int i = 0; i < n_rows; i++) {
        ...
        pax_draw_text(&fb, COL_WHITE, FONT, TXT_TINY,
                      panel_x + 30, row_cy - TXT_TINY / 2 - 1, items[i].l);
    }
}
...
static void render_node_pins(double ctr_lat, double ctr_lon, int zoom,
                             int map_x0, int map_y0, int map_w, int map_h) {
    ...
    for (int i = 0; i < node_count; i++) {
        ...
        int fx, fy;
        if (!latlon_to_fb(lat, lon, ctr_lat, ctr_lon, zoom,
                          map_x0, map_y0, map_w, map_h, &fx, &fy)) {
            continue;
        }
    ...
    if (best_name[0] && best_d2 < 80 * 80) {
        ...
        int ly = best_fy + 8;
        int pad = 3;
        pax_simple_rect(&fb, 0xC0000000,
                        lx - pad, ly - pad, (int)nsz.x + 2 * pad, TXT_SMALL + 2 * pad);
