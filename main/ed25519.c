...
{
    uint8_t nonce[64];  /* SHA-512(nonce_prefix || message) */
    uint8_t hram[64];   /* SHA-512(R || A || M)             */
    uint8_t r[64];      /* nonce reduced mod l              */
    ge_p3   R;

    /* r = SHA-512(private_key[32..63] || message) mod l */
    sha512_2(private_key + 32, 32, message, message_len, nonce);
    memcpy(r, nonce, 64);
    sc_reduce(r);           /* overwrites lower 32 bytes with r mod l */

    /* R = r * B  (stored in signature[0..31]) */
    ge_scalarmult_base(&R, r);
    ge_p3_tobytes(signature, &R);

    /* k = SHA-512(R || public_key || message) mod l */
    sha512_3(signature, 32, public_key, 32, message, message_len, hram);
    sc_reduce(hram);        /* lower 32 bytes = k mod l */
