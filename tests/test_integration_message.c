...
static void tx_build(const uint8_t* key, uint32_t ts, uint8_t text_type, const char* text, uint8_t channel_hash,
...
static bool rx_decode(uint8_t* wire, uint8_t wire_size, const uint8_t* key, meshcore_grp_txt_t* out_grp) {
    meshcore_message_t msg;
