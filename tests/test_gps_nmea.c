...
int main(void) {
    ...
    EXPECT(abs(st.lat_e6 - exp_lat_e6) <= 2, "lat_e6 expected ~%d, got %ld", exp_lat_e6, (long)st.lat_e6);
    EXPECT(abs(st.lon_e6 - exp_lon_e6) <= 2, "lon_e6 expected ~%d, got %ld", exp_lon_e6, (long)st.lon_e6);
    ...
    //         state untouched.
    memset(&st, 0, sizeof(st));
    bool parsed_bad = gps_nmea_apply_line("$GNRMC,140555.000,A,5152.3082,N,00517.4878,E,0.31,54.45,080625,,,A*00", &st);
    ...
    memset(&st, 0, sizeof(st));
    bool parsed_no_cs = gps_nmea_apply_line("$GNRMC,140555.000,A,5152.3082,N,00517.4878,E,0.31,54.45,080625,,,A", &st);
