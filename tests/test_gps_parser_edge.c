...
int main(void) {
    ...
    memset(&st, 0, sizeof(st));
    bool rmc_void = feed(&st, "GNRMC,140555.000,V,5152.3082,N,00517.4878,E,0.31,54.45,080625,,,N");
    ...
    memset(&st, 0, sizeof(st));
    bool gga_nofix = feed(&st, "GNGGA,140556.000,,,,,0,00,99.99,,M,,M,,");
    ...
    EXPECT(st.lon_e6 > 0, "E longitude must be positive, got %ld", (long)st.lon_e6);
    {
        ...
        EXPECT(abs(st.lat_e6 - exp_e6) <= 2, "S lat_e6 expected ~%d, got %ld", exp_e6, (long)st.lat_e6);
    }
    ...
    EXPECT(st.lon_e6 < 0, "W longitude must be negative, got %ld", (long)st.lon_e6);
    {
        ...
        EXPECT(abs(st.lon_e6 - exp_e6) <= 2, "W lon_e6 expected ~%d, got %ld", exp_e6, (long)st.lon_e6);
    }
    ...
    EXPECT(st.fix_valid, "near-pole fix must be valid");
    {
        ...
        EXPECT(abs(st.lat_e6 - exp_lat) <= 2, "near-pole lat_e6 expected ~%d, got %ld", exp_lat, (long)st.lat_e6);
        EXPECT(abs(st.lon_e6 - exp_lon) <= 2, "near-antimeridian lon_e6 expected ~%d, got %ld", exp_lon,
               (long)st.lon_e6);
    ...
    memset(&st, 0, sizeof(st));
    EXPECT(!feed(&st, "GNRMC,140555.000,A,5152.3082"), "short RMC (nf<7) must be ignored");
    EXPECT(!feed(&st, "GNGGA,140556.000,5152.3083,N,00517.4878"), "short GGA (nf<9) must be ignored");
    EXPECT(!feed(&st, "GPGSV,1,1"), "short GSV (nf<4) must be ignored");
