...
int main(void) {
    ...
    EXPECT(nl && strcmp(nl->display_name, "Netherlands") == 0, "NL display_name");
    ...
    EXPECT(unset != NULL && unset->n_subbands == 0, "unset sentinel present with 0 sub-bands");
    EXPECT(region_match_subband(unset, 869.618f) == NULL, "unset sentinel matches no band");
    ...
    EXPECT(region_get_country_by_index(REGION_COUNTRY_COUNT) == NULL, "out-of-range index → NULL");
    ...
    EXPECT(g3 && strcmp(g3->label, "g3") == 0, "869.618 → g3, got %s", band_at(nl, 869.618f));
    ...
    EXPECT(region_effective_power_dbm(NULL, 17, 5) == 17, "NULL country returns conducted power unchanged");
    ...
    EXPECT(region_dc_budget_ms_per_hour(region_match_subband(nl, 866.0f)) == 36000u, "1%% (g1) → 36000 ms/h");
    ...
    for (int i = 0; i < REGION_COUNTRY_COUNT; i++) {
        ...
        EXPECT(c && c->country_code && c->display_name, "country %d has code + name", i);
        ...
        for (uint8_t b = 0; b < c->n_subbands; b++) {
            ...
            EXPECT(sb->freq_min_mhz <= sb->freq_max_mhz, "%s/%s freq_min ≤ freq_max", c->country_code, sb->label);
            EXPECT(sb->duty_cycle_permille >= 1 && sb->duty_cycle_permille <= 1000, "%s/%s duty in 1..1000",
                   c->country_code, sb->label);
        }
    }
