...
int main(void) {
    ...
    EXPECT(eu433 && strcmp(eu433->display_name, "EU 433 MHz") == 0, "mixed-case eU-433 resolves");
    ...
    EXPECT(be && be->subbands == de->subbands, "EU868 countries share one sub-band table");
    ...
    EXPECT(region_get_country_by_index(REGION_COUNTRY_COUNT - 1) != NULL, "last index valid");
    EXPECT(region_get_country_by_index(REGION_COUNTRY_COUNT) == NULL, "one past the end → NULL");
    ...
    for (int i = 0; i < REGION_COUNTRY_COUNT; i++) {
        if (region_get_country_by_index(i) == jp_by_iso) {
            jp_idx = i;
            break;
        }
    }
    ...
    EXPECT(jp && jp->subbands[0].lbt_alternative == true, "JP band carries LBT-required flag");
    ...
    EXPECT(region_match_subband(ru, 867.0f) == NULL, "RU 867.0 falls in the inter-window gap");
    ...
    EXPECT(region_match_subband(au, 902.0f) == NULL, "AU 902.0 below ANZ915 lower edge → NULL");
    ...
    EXPECT(region_effective_power_dbm(NULL, 27, 9) == 27, "NULL country → conducted power unchanged");
