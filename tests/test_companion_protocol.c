...
static void capture_cb(companion_command_packet_t *cmd,
...
int main(void) {
    ...
    EXPECT(cb_cmd.command == COMPANION_CMD_SET_ADVERT_LATLON,
           "opcode expected 14, got %d", (int)cb_cmd.command);
    EXPECT(cb_cmd.command_set_advert_latlon_args.latitude == exp_lat,
           "lat expected %d, got %d", exp_lat, cb_cmd.command_set_advert_latlon_args.latitude);
    EXPECT(cb_cmd.command_set_advert_latlon_args.longitude == exp_lon,
           "lon expected %d, got %d", exp_lon, cb_cmd.command_set_advert_latlon_args.longitude);
    ...
    EXPECT(cb_cmd.command_set_advert_latlon_args.latitude == exp_lat,
           "lat after chunked feed expected %d, got %d",
    ...
    EXPECT(cb_cmd.command_set_advert_latlon_args.latitude == bb_lat,
           "negative lat survival expected %d, got %d",
           bb_lat, cb_cmd.command_set_advert_latlon_args.latitude);
    EXPECT(cb_cmd.command_set_advert_latlon_args.longitude == bb_lon,
           "lon byte-by-byte expected %d, got %d",
           bb_lon, cb_cmd.command_set_advert_latlon_args.longitude);
    ...
    EXPECT(err == COMPANION_COMMAND_PARSER_ERROR_NONE,
           "direct parse err=%d", err);
    EXPECT(pkt.command == COMPANION_CMD_SET_ADVERT_LATLON,
           "direct opcode expected 14, got %d", (int)pkt.command);
    EXPECT(pkt.command_set_advert_latlon_args.latitude == direct_lat,
