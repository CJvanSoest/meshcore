...
| `http_api_key` | str (64) | Shared secret for the HTTPS `/ping` endpoint. Auto-generated 64 hex chars on first boot if missing. Rotated by **Settings → HTTPS → Regenerate API key**. |
...
Both wiped + regenerated by **Settings → HTTPS → Regenerate cert**.
