...
| `gps_src` | u8 | Source tag: 0=none, 1=manual, 2=PA1010D, 3=USB-CDC, 4=BLE |
| `ble_enabled` | u8 | 0/1 — NimBLE companion radio on next start |
| `ble_gps_pref` | u8 | iPhone-app mirror of "share my position in adverts" |
| `ble.pin` | u32 | Fixed 6-digit NimBLE pairing passkey (0..999999, shown as `%06u`); injected via `ble_sm_inject_io`. Default 0 (`000000`) until set in **Settings → Bluetooth → Pairing code** |
