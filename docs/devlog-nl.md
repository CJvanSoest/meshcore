...
## Update — v2.2.0 sprint: tile-home + render-split + per-app brightness (juni 2026)

Een volledige UI-restyle naar een tile-grid landing in Pager-stijl, Settings opgesplitst in een tweelaags menu (categorie-tiles → drilldown), een nieuwe Brightness-categorie, en een About-tile met live versie. En tegelijkertijd: de Gitea-forks van `tanmatsu-radio` en `tanmatsu-lora` opgeruimd toen `rx_boost` upstream landde in lora v0.3.0 / radio v3.2.0.

🧱 **Refactor vóór je gaat uitbreiden bespaart later twee keer zoveel.** Mijn `render.c` was 1.353 regels toen ik wist dat ik VIEW_HOME, VIEW_ABOUT en op termijn VIEW_MAP nodig zou hebben. De verleiding was direct beginnen met de nieuwe view en de oude monoliet aanvullen — sneller in de eerste minuut, langzamer in elke minuut daarna. In plaats daarvan eerst splitsen: per view een eigen file, `render_internal.h` voor cross-file declaraties, dispatcher in `render.c`. Pas daarna `render_home.c` toegevoegd. Les: als je weet dat er meer komt is een refactor géén yak-shave — het is de eerste van de N nieuwe features die je gaat bouwen. De tijd die je erin steekt deel je door N.

🔁 **Eén-regel-fixes die hele bugklassen opruimen vind je door naar het systeem te kijken, niet naar de bug.** De QR-overlay flickerde en de emoji-picker ook. Twee aparte bugs leek het. Tot ik naar `render()` keek en zag dat elke `render_*()` zelf een `blit()` deed, en de dispatcher er een tweede overheen plakte voor de overlay. Twee blits per frame = base-laag zichtbaar tussen de twee = flicker. Fix: blit uit alle render_* halen, dispatcher doet er één aan het einde. Geen verschil voor de individuele scènes, maar de hele "overlay flickert" categorie is weg. Les: bug-classes zitten vaker in invarianten van het systeem dan in losse plekken. Als twee onafhankelijke features dezelfde "kleine" glitch hebben, kijk niet naar de features — kijk naar wat ze delen.

🔌 **Niet elke backlog-feature heeft een schone trigger; accepteer dat je platform iets niet kan.** Brightness #47 vroeg om "restore the launcher's values when MeshCore exits". Maar BadgeVMS PIE ELF apps hebben geen netto exit-hook — de gebruiker drukt een knop en de launcher restart het OS. Geen `atexit`, geen "on suspend", geen signal-handler. Ik heb de restore-poging overgeslagen en in de wiki + commit gezet *waarom* niet. CJ's veldtest een uur later: "in launcher 30%, in meshcore 5%, app sluiten — launcher blijft 30%, app weer open — 5%, precies goed". Les: als het platform iets niet kan, verzin dan geen halve oplossing. Schrijf op waarom je 'm overslaat, en laat de gebruiker bevestigen dat het zo geaccepteerd is.

📐 **Hiërarchische UI schaalt lineair met features; platte UI hit een aandachtsplafond.** v2.1.x had één Settings-tab met 19 fields scrollend onder elkaar. Daar pasten 3 extra fields voor brightness niet bij — niet technisch, qua *vindbaarheid*. Tile-grid → drilldown: 6 categorie-tiles, drill in en je ziet alleen die 3 fields. De gebruiker hoeft geen 22-rijen-lijst meer te scannen. Les: navigatie-patronen hebben een schaal-limiet. Een tab-bar werkt tot 4-5 tabs, een flat list tot ~15 items; daarboven moet je structuur in de UI brengen of de gebruiker verliest het overzicht. Net zoals code-bestanden: een 1.300-regels file is hetzelfde probleem op een andere as.

🔓 **Wachten loont — als je tegelijk een tussenoplossing levert.** De `rx_boost` PR die ik in juni 2026 opende, was bedoeld als upstream-fix. Hij landde 11 dagen later. In die tijd draaide mijn app op een Gitea-fork van `tanmatsu-lora`, met een fallback-parser die zowel de oude als nieuwe response-lengte accepteerde. Toen v0.3.0 upstream gepubliceerd werd kon ik in één commit terug naar de IDF component registry; CJ Gitea-forks van zowel `tanmatsu-radio` als `tanmatsu-lora` zijn nu nul-delta met upstream en gearchiveerd. Les: de optimale strategie bij upstream-afhankelijkheden is *parallel* werken — PR voor de echte fix, lokaal een tolerante shim. Beide horen erbij; één zonder de ander betekent of wachten op iemand anders, of permanent een fork-onderhoudslast.

## Update — ABI breken kost users, niet jou (juni 2026)

Drie dagen na v2.1.1 mailde Renze: na het flashen van zijn stock-upstream firmware kreeg hij `lora_get_config: Invalid response length: 24` en "C6 unavailable" op mijn appstore-versie. Mijn fork van de LoRa-component had ergens onderweg een `bool rx_boost` veld aan de config-struct toegevoegd; struct werd 16 → 17 bytes, response van de co-processor werd 24 → 25. Mijn app verwacht 25, stock firmware levert 24, strict size-check zegt nee — handle wordt niet geïnitialiseerd, geen radio meer.

🧱 **Een ABI-uitbreiding aan een struct is een breaking change, ook als hij "alleen iets toevoegt".** Achter aan een packed struct een nieuwe boolean plakken voelt onschuldig — geen oude bits verschoven, geen semantiek veranderd. Maar wire-protocol-code aan de andere kant van de pipe doet `if (response_length != sizeof(struct))` of erger `<` — niet `>=`. Zodra mijn fork-app op stock firmware draait of v.v., crashen ze allebei op precies die check. Les: zodra je een struct over een wire-grens transporteert is hij feitelijk een protocol, en groei van een protocol vereist altijd ofwel versioning, ofwel een tolerante parser, ofwel een aparte cmd-byte. Een veld er stilletjes aan plakken is een belofte aan alle huidige consumers die niemand zwart op wit gemaakt heeft.

📦 **Een appstore vergroot de blast-radius van elke ABI-keuze.** Toen mijn app alleen op mijn eigen hardware draaide was die struct-keuze prima — beide kanten waren altijd uit dezelfde commit gebouwd. Zodra ik publiceerde naar `apps.tanmatsu.cloud` werd elke combinatie van (mijn app) × (welke firmware-versie de user draait) een echte deployment. Wat eerst een impliciete aanname was — "we gebruiken samen mijn fork" — werd een impliciete *eis*, en eisen die je niet zichtbaar maakt, voldoet niemand aan. Les: publiceren betekent toegeven dat je consumers buiten je controle zijn — controleer expliciet je aannames over hun environment, of bouw je app tolerant genoeg dat 't niet uitmaakt.

🌉 **Een fallback-parser koop je terwijl je op upstream wacht.** Idealiter was die `rx_boost` gewoon upstream — maar Renze's repo werkt in z'n eigen tempo en mijn users zitten nu in de prut. Ik heb daarom twee dingen tegelijk gedaan: een PR ingediend bij upstream (struct match weer aligned op de bron), én m'n eigen fork een tolerante parser gegeven (accepteer 24 OF 25 bytes, default `rx_boost = false` op de korte vorm). De PR is de juiste lange-termijn-fix; de fallback is een bridge die *vandaag* werkt. Les: bij community-stack-bugs is "open een PR" zelden snel genoeg — een tweesporenoplossing (upstream-PR + lokale defensieve patch) ontkoppelt jouw recovery-tijd van iemand anders' review-cyclus.

## Update — Sprint #1 professionalisering: CI op de NAS + PR-flow erin (juni 2026)

Na v2.2.0 had ik 't gevoel dat ik de losse-commits-naar-main flow ontgroeid was — niet omdat 't faalde, maar omdat er bij elke release-week eerder een "oh, vergeten te builden voor de push" of "wacht, ik had eerst even moeten checken of de wire-format nog matchte" geweest was. Vier achtereenvolgende sessies klein-incident-debuggen. Tijd om de gate hard te maken.

🏗 **Je CI-image is jouw artefact, niet die van de upstream.** Ik wilde Gitea Actions opzetten met de officiële `espressif/idf:v5.5.1` container — IDF + toolchain pre-installed, leek perfect. Eerste run faalde meteen: `actions/checkout@v4` is een JavaScript-action en de Espressif-image ship'd geen Node. Drie opties: terugvallen op `runs-on: ubuntu-latest` en IDF zelf installeren (10 min per build), elk gebruik van JS-actions vermijden (forever taxen op de devops-cultuur), of m'n eigen afgeleide image maken. Ik koos 't laatste: `FROM espressif/idf:v5.5.1 + RUN apt install nodejs`, gebouwd op de NAS, lokaal getagd. Les: zodra je CI in een container draait is die image een artefact dat je zelf onderhoudt — wachten tot upstream jouw use-case fixt is de duurste optie.

🌐 **Per-job networking isoleert by default; expliciete bridges verbinden.** Tweede run: image werkt, container start, checkout begint, `git fetch http://gitea:3000/...` → "Could not resolve host: gitea". De act_runner spawnt elke job in z'n eigen wegwerp-netwerk om jobs te isoleren — maar daarmee staat 'ie ook los van het netwerk waar Gitea zelf draait. Fix: `container.network: gitea_default` in de runner-config, en jobs draaien voortaan op hetzelfde netwerk als Gitea. Geen Docker-magie nodig in de workflow zelf, gewoon één regel infra-config. Les: container-isolatie is een feature totdat 't een muur is — wees expliciet over welke services binnen jouw "binnen" vallen.

🚪 **Branch protection maakt 'ik zou moeten' tot 'ik moet'.** CI alleen is een notificatie, geen poort. Zolang ik nog `git push gitea main` kan tikken is groen-of-rood advies, niet beleid. Echte gate: branch protection op `main`, direct-push uit, `CI / Build (tanmatsu) (pull_request)` als required check, block-on-outdated aan. Eerste echte PR ooit op deze repo (`#1`, ci-workflow zelf) — voelde alsof ik m'n eigen huis op slot draaide, maar precies daar zit de waarde: je kunt niet langer per ongeluk een rode commit landen. Voor solo dev is de PR-flow geen ceremonie, het is een **discipline-scaffolding**. Devlogs (deze tekst dus) blijven exempt via `unprotected_file_patterns` — anders ben ik m'n eigen notities-werkflow aan 't kapot-bureaucratiseren.

Eindstaat: Gitea Actions runner draait permanent op de NAS, elke push triggert een build, elke PR krijgt een groene-of-rode vink voor de merge mogelijk is, en de eerste twee PR's (`#1` CI, `#2` PR-template) zijn gemerged via de nieuwe flow. Wat erna landt op `main` is per definitie een keer gebouwd door iets anders dan mijn lokale Mac — precies wat ik miste.

## Update — v2.3.x sprint: F3 blank + RTC-tijd + preamble-fix + auto-screensaver (juni 2026)

Korte sprint deels op feedback van de eerste echte user (nica-f op GitHub issue #1), deels op een hint van de firmware-maintainer (Nicolai's tip dat de launcher al SNTP doet en de tijd op de C6 RTC staat). Drie features uitgerold, één stille protocol-bug-jacht die nog niet rond is, en één debug-konijnenhol dat meer tijd kostte dan de features bij elkaar.

⏱ **Een protocol-bug overleeft omdat de happy-path zichzelf valideert.** `send_advert` schreef `advert.timestamp = xTaskGetTickCount() * portTICK_PERIOD_MS / 1000` — uptime sinds boot, geen UNIX-epoch. Lokaal leek alles fine: m'n eigen RX-pad valideert de timestamp niet tegen wall-clock, en de signature die de sender berekent dekt diezelfde foute waarde af, dus de handtekening klopt voor zichzelf. Andere clients leggen een freshness-window over de timestamp en droppen alles wat zegt uit 1970 te komen. De bug zat er maandenlang in zonder dat ik 'm opmerkte, omdat de zender nooit feedback krijgt over de afwijzing. Les: zodra je een "tijd"-veld serialiseert, audit *elke* encoder in de codebase op dezelfde conventie. `time(NULL)` en `xTaskGetTickCount` door elkaar gebruiken is het soort inconsistentie dat alleen opvalt als iemand anders' strengere validator in de loop zit. Goeie test: lees je eigen packets terug met een *vreemde* parser, niet alleen de lokale — je eigen code is een mede-samenzweerder.

📸 **Een door-de-user-opgenomen packet capture is de goedkoopste debug-data die je gaat krijgen.** De rapporteur draaide een Waveshare LoRa USB-stick náást z'n TDeck+ / T1000-E / Xiao ontvangers en plakte een 110-byte hex dump van m'n Tanmatsu-advert in de GitHub-issue. Binnen tien minuten lezen tegen onze `meshcore_serialize` layout had ik de timestamp-anomalie te pakken: bytes [34..37] = `0F 00 00 00` = 15 seconden = uptime. Geen hoeveelheid "check region scope, check antenne, check sensitivity" had dat zonder die bytes laten zien. Les: bij protocol-bugs altijd om de raw packet vragen vóór je gaat gokken. Kost de rapporteur één capture; bespaart jou uren speculatie. Bouw issue-templates die hier naar hinten. En als je stack geen in-app packet-sniffer view heeft is dat plotseling de hoogst-leverage feature die je kunt toevoegen — vandaar dat het nu issue #3 op deze repo is.

🛠 **Build-state drift is onzichtbaar voor `git diff`.** Een halve dag bezig met testen wat ik dacht dat de "geen-WiFi, RTC-tijd"-change was, faalden alle receiver-side checks: `wifi init: FAIL`, `lora_init_remote: OK` gevolgd door "C6 unavailable", whole package. Ik dacht dat de runtime-change kapot was. Pas na revert naar een build identiek aan `main` HEAD en *opnieuw* falen, vergeleek ik m'n lokale `sdkconfig_tanmatsu` met een werkende worktree: m'n build had `CONFIG_ESP_HOSTED_CP_TARGET_ESP32H2` + `CONFIG_ESP_HOSTED_SPI_HOST_INTERFACE`, de werkende dir had `..._C6` + `..._SDIO`. Tijdens 't experiment had ik `tanmatsu-wifi` + `wifi-manager` in `idf_component.yml` getoggeld en idf.py had toen de gecombineerde sdkconfig opnieuw afgeleid uit Kconfig-defaults — die voor Tanmatsu niet kloppen. `git diff` was schoon want `sdkconfig_*` is gitignored. Les: als "alles kapot is" en de source aantoonbaar ongewijzigd, verdenk dan eerst de gegenereerde config voor je naar de code kijkt. Recovery: `rm -f sdkconfig_<device> && rm -rf build/<device> && make build`. Memory-entry waard, want dit komt terug zodra deps weer schuiven.

🔁 **Een app-store-PR is een long-lived contract, geen per-release event.** Bij Nicolai's app-repository stond een PR open die de manifest van v2.1.2 naar v2.2.0 bumpte. Halverwege de sprint sneed ik v2.3.0 (losse features), daarna v2.3.1 (auto-screensaver follow-up). De reflex was "sluit die PR, open een nieuwe voor v2.3.1". In plaats daarvan: dezelfde branch ge-amended, metadata naar v2.3.1 / revision 6, nieuwe binary erin, force-push, en de PR-titel + body via de GitHub API gepatched. Zelfde review-thread, zelfde maintainer-gesprek, zelfde context — de maintainer behoudt de geschiedenis. Sluiten en heropenen had die eerdere discussie begraven. Les: een app-store-PR is het langstlevende artefact in de release-pipeline. Optimaliseer voor de aandacht van de reviewer, niet voor jouw release-cadens. Heb je twee releases in één dag uitgesneden? De reviewer wil geen twee PRs — die wil één PR waar 'ie één keer naar kan kijken.
