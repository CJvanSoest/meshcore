...
static bool wav_open(const char* path, FILE** out_f, uint32_t* out_rate, uint16_t* out_channels,
    ...
    if (!have_fmt || !have_data || fmt.format != 1 || fmt.bits_per_sample != 16) {
        ESP_LOGW(TAG, "%s: unsupported WAV (fmt=%u bits=%u)", path, fmt.format, fmt.bits_per_sample);
        fclose(f);
        return false;
    }
    *out_f         = f;
...
static void stream_wav(FILE* f, uint16_t channels, uint32_t data_size) {
    ...
    while (bytes_left > 0) {
        ...
        size_t written = 0;
        i2s_channel_write(s_i2s, out_buf, out_frames * 2 * sizeof(int16_t), &written, pdMS_TO_TICKS(500));
    }
    ...
    while (tail_frames > 0) {
        ...
        size_t written = 0;
        i2s_channel_write(s_i2s, out_buf, batch * 2 * sizeof(int16_t), &written, pdMS_TO_TICKS(200));
        tail_frames -= batch;
    }
    free(in_buf);
    free(out_buf);
...
static void play_task(void* arg) {
    ...
    if (wav_open(pa->path, &f, &rate, &channels, &data_size)) {
        ...
        ESP_LOGI(TAG, "play %s (%uHz %uch %u B)", pa->path, (unsigned)rate, (unsigned)channels, (unsigned)data_size);
...
void sounds_play_dm(void) {
    if (sound_volume_pct) play_async(sound_dm_slot);
}
void sounds_play_channel(void) {
    if (sound_volume_pct) play_async(sound_channel_slot);
}
void sounds_play_error(void) {
    if (sound_volume_pct) play_async(sound_error_slot);
}
void sounds_play_boot(void) {
    if (sound_volume_pct) play_async(sound_boot_slot);
}
