...
esp_err_t cert_gen_self_signed(char *cert_pem, size_t cert_cap,
    ...
    rc = mbedtls_ctr_drbg_seed(&drbg, esp_rng_cb, NULL,
                               (const unsigned char *)PERS, strlen(PERS));
    if (rc != 0) { ESP_LOGE(TAG, "drbg seed: -0x%04x", -rc); result = ESP_FAIL; goto out; }

    // ECDSA P-256 key. ~1-2 s on ESP32-P4 the first time.
    rc = mbedtls_pk_setup(&pk, mbedtls_pk_info_from_type(MBEDTLS_PK_ECKEY));
    if (rc != 0) { ESP_LOGE(TAG, "pk_setup: -0x%04x", -rc); result = ESP_FAIL; goto out; }
    rc = mbedtls_ecp_gen_key(MBEDTLS_ECP_DP_SECP256R1, mbedtls_pk_ec(pk),
                             mbedtls_ctr_drbg_random, &drbg);
    if (rc != 0) { ESP_LOGE(TAG, "ecp_gen_key: -0x%04x", -rc); result = ESP_FAIL; goto out; }
    ...
    if (rc != 0) { ESP_LOGE(TAG, "set_subject: -0x%04x", -rc); result = ESP_FAIL; goto out; }
    rc = mbedtls_x509write_crt_set_issuer_name(&crt, SUBJ);
    if (rc != 0) { ESP_LOGE(TAG, "set_issuer: -0x%04x", -rc); result = ESP_FAIL; goto out; }
    ...
    if (rc != 0) { ESP_LOGE(TAG, "set_serial_raw: -0x%04x", -rc); result = ESP_FAIL; goto out; }
    ...
    rc = mbedtls_x509write_crt_set_validity(&crt,
                                            "20260101000000",
                                            "20360101000000");
    if (rc != 0) { ESP_LOGE(TAG, "set_validity: -0x%04x", -rc); result = ESP_FAIL; goto out; }

    rc = mbedtls_x509write_crt_set_basic_constraints(&crt, 1, 0);  // CA:TRUE
    if (rc != 0) { ESP_LOGE(TAG, "set_basic: -0x%04x", -rc); result = ESP_FAIL; goto out; }
    rc = mbedtls_x509write_crt_set_key_usage(&crt,
        MBEDTLS_X509_KU_DIGITAL_SIGNATURE | MBEDTLS_X509_KU_KEY_ENCIPHERMENT |
        MBEDTLS_X509_KU_KEY_CERT_SIGN);
    if (rc != 0) { ESP_LOGE(TAG, "set_key_usage: -0x%04x", -rc); result = ESP_FAIL; goto out; }
    ...
    if (rc != 0) { ESP_LOGE(TAG, "set_san: -0x%04x", -rc); result = ESP_FAIL; goto out; }

    rc = mbedtls_x509write_crt_pem(&crt, (unsigned char *)cert_pem, cert_cap,
                                   mbedtls_ctr_drbg_random, &drbg);
    if (rc != 0) { ESP_LOGE(TAG, "crt_pem: -0x%04x", -rc); result = ESP_FAIL; goto out; }

    rc = mbedtls_pk_write_key_pem(&pk, (unsigned char *)key_pem, key_cap);
    if (rc != 0) { ESP_LOGE(TAG, "pk_pem: -0x%04x", -rc); result = ESP_FAIL; goto out; }

    ESP_LOGI(TAG, "generated ECDSA P-256 self-signed cert (cert=%u B, key=%u B)",
             (unsigned)strlen(cert_pem), (unsigned)strlen(key_pem));
...
esp_err_t cert_gen_load_or_create(char *cert_pem, size_t cert_cap,
    ...
    if (save_err != ESP_OK) {
        ESP_LOGW(TAG, "cert generated but NVS save failed: 0x%x — will regen next boot",
                 save_err);
    }
    return ESP_OK;
}
...
esp_err_t cert_gen_fingerprint_hex(const char *cert_pem,
    ...
    int rc = mbedtls_x509_crt_parse(&crt, (const unsigned char *)cert_pem,
                                    strlen(cert_pem) + 1);
    if (rc != 0) {
