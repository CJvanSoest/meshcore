...
// pattern for fields that are just "one u8 / u16 / u32 keyed under a
// namespace".
...
bool nvs_load_u16(const char *ns, const char *key, uint16_t *out);
bool nvs_load_u32(const char *ns, const char *key, uint32_t *out);
bool nvs_load_i8 (const char *ns, const char *key, int8_t   *out);
bool nvs_load_i32(const char *ns, const char *key, int32_t  *out);
...
bool nvs_save_u16(const char *ns, const char *key, uint16_t val);
bool nvs_save_u32(const char *ns, const char *key, uint32_t val);
bool nvs_save_i8 (const char *ns, const char *key, int8_t   val);
bool nvs_save_i32(const char *ns, const char *key, int32_t  val);
