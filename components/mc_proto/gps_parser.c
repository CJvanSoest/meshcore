...
bool gps_nmea_apply_line(const char* line, gps_status_t* st) {
    ...
    if (strcmp(type, "RMC") == 0 && nf >= 7) {
        ...
        if (f[2][0] == 'A') {
            ...
            if (dmm_to_e6(f[3], f[4][0], &lat_e6) && dmm_to_e6(f[5], f[6][0], &lon_e6)) {
                st->lat_e6    = lat_e6;
                st->lon_e6    = lon_e6;
                st->fix_valid = true;
            }
    ...
    if (strcmp(type, "GGA") == 0 && nf >= 9) {
        ...
        if (st->fix_quality > 0 && f[2][0]) {
            int32_t lat_e6, lon_e6;
            if (dmm_to_e6(f[2], f[3][0], &lat_e6) && dmm_to_e6(f[4], f[5][0], &lon_e6)) {
                st->lat_e6    = lat_e6;
                st->lon_e6    = lon_e6;
                st->fix_valid = true;
            }
    ...
    if (strcmp(type, "GSV") == 0 && nf >= 4) {
        ...
        int sats = atoi(f[3]);
        if (tlen >= 5 && f[0][1] == 'G' && f[0][2] == 'P')
            st->gps_sats_view = sats;
        else if (tlen >= 5 && f[0][1] == 'G' && f[0][2] == 'L')
            st->glo_sats_view = sats;
        return true;
    }
