...
void emoji_init(void) {
    ...
    for (int i = 0; i < EMOJI_COUNT; i++) {
        pax_buf_init(&emoji_bufs[i], (void*)(uintptr_t)EMOJI_BITMAPS[i], EMOJI_BITMAP_SIZE, EMOJI_BITMAP_SIZE,
                     PAX_BUF_32_8888ARGB);
    }
...
void emoji_draw(int idx, int cx, int cy, int radius, pax_buf_t* fb) {
    ...
    pax_draw_image_sized(fb, &emoji_bufs[idx], (float)(cx - radius), (float)(cy - radius), d, d);
}
...
static int flush_text_run(pax_buf_t* fb, pax_col_t col, const pax_font_t* font, float size, int x, int y, char* buf,
                          int* blen) {
    if (*blen == 0) return 0;
    buf[*blen]   = '\0';
...
static int render_or_measure(pax_buf_t* fb, pax_col_t col, const pax_font_t* font, float size, int x, int y,
                             const char* text) {
    int dx = 0;
