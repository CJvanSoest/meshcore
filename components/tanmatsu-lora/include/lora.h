...
typedef enum {
    ...
    LORA_PROTOCOL_TYPE_PACKET_RX_V2   = 0x09,
    LORA_PROTOCOL_TYPE_GET_RSSI_INST  = 0x0A,
...
typedef struct {
    uint8_t rssi_pkt_raw;        // RssiPkt: dBm = -raw/2
    int8_t  snr_pkt_raw;         // SnrPkt:  dB  = raw/4 (signed)
    uint8_t signal_rssi_pkt_raw; // SignalRssiPkt: dBm = -raw/2
    bool    valid;               // false als C6 firmware geen V2 ondersteunt
...
// Returnt instantane RSSI in raw byte (dBm = -raw/2). Geeft ESP_ERR_INVALID_RESPONSE als C6 firmware
// het type niet ondersteunt (oude firmware → NACK).
