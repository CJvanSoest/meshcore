...
const char* gps_profile_label(gps_profile_t p) {
    switch (p) {
        case GPS_PROFILE_WALKING:
            return "Walking";
        case GPS_PROFILE_CYCLING:
            return "Cycling";
        case GPS_PROFILE_DRIVING:
            return "Driving";
        case GPS_PROFILE_MANUAL:
            return "Manual";
        default:
            return "?";
    }
...
static float distance_m(int32_t lat_a_e6, int32_t lon_a_e6, int32_t lat_b_e6, int32_t lon_b_e6) {
...
static void task_loop(void* arg) {
    ...
    while (1) {
        ...
        if (!s_have_last_commit) {
            ...
            float d_m = distance_m(s_last_commit_lat_e6, s_last_commit_lon_e6, st.lat_e6, st.lon_e6);
            commit    = (d_m >= (float)commit_m) || (now - s_last_commit_ms >= (uint32_t)GPS_HEARTBEAT_MAX_S * 1000u);
        }
        if (commit) {
            ...
            ESP_LOGI(TAG, "commit lat=%ld lon=%ld sats=%u hdop=%.2f", (long)st.lat_e6, (long)st.lon_e6,
