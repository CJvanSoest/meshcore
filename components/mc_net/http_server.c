...
static esp_err_t ping_post_handler(httpd_req_t* req) {
    ...
    char hdr[80] = {0};
    bool authed  = false;
    if (httpd_req_get_hdr_value_str(req, "X-API-Key", hdr, sizeof(hdr)) == ESP_OK && strcmp(hdr, http_api_key) == 0) {
        authed = true;
    }
    if (!authed) {
    ...
    if (cJSON_IsArray(data)) {
        ...
        if (cJSON_IsNumber(lat) && cJSON_IsNumber(lon)) {
            ...
            la = atof(lat->valuestring);
            lo = atof(lon->valuestring);
            ok = true;
        }
        if (ok && la >= -90.0 && la <= 90.0 && lo >= -180.0 && lo <= 180.0) {
            ...
            if (cJSON_IsNumber(ts))
                best_ts = (int64_t)ts->valuedouble;
            else if (cJSON_IsString(ts))
                best_ts = (int64_t)atoll(ts->valuestring);
            else
                best_ts = 0;
            valid = 1;
        }
    ...
    save_gps_coords();
    ESP_LOGI(TAG, "/ping accepted: lat=%.6f lon=%.6f ts=%lld (from %d entries)", best_lat, best_lon, (long long)best_ts,
             valid);
...
static esp_err_t cert_get_handler(httpd_req_t* req) {
    ...
    httpd_resp_set_hdr(req, "Content-Disposition", "attachment; filename=\"tanmatsu.crt\"");
...
static void cert_ensure_loaded(void) {
    ...
    esp_err_t err = cert_gen_load_or_create(s_cert_pem, sizeof(s_cert_pem), s_key_pem, sizeof(s_key_pem));
    if (err != ESP_OK) {
...
static void server_start(void) {
    ...
    mdns_ensure_started();
    ESP_LOGI(TAG, "HTTPS server up on :%d (cert=%u B, key=%u B)", cfg.port_secure, (unsigned)strlen(s_cert_pem),
