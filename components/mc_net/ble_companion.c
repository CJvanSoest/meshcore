...
static int rx_chr_access(uint16_t conn_handle, uint16_t attr_handle,
    ...
    ESP_LOGI(TAG, "rx_chr_access write %u B: %02X %02X %02X %02X ...",
             copied, buf[0], copied > 1 ? buf[1] : 0,
...
static int gap_event(struct ble_gap_event *event, void *arg) {
    ...
    switch (event->type) {
        ...
        case BLE_GAP_EVENT_ENC_CHANGE: {
            ...
            if (rc == 0) {
                ESP_LOGI(TAG, "enc_change status=%d enc=%u auth=%u bonded=%u key_size=%u",
                ...
                ESP_LOGI(TAG, "enc_change status=%d (conn_find rc=%d)",
        ...
        case BLE_GAP_EVENT_SUBSCRIBE:
            ESP_LOGI(TAG, "subscribe handle=%u cur_notify=%d", event->subscribe.attr_handle, event->subscribe.cur_notify);
            break;
